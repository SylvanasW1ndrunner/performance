...
async function loadCurrentUserInfo() {
    try {
        ...
        document.querySelector('.department-info span:first-child').textContent = `${userData.team} - ${userData.subTeam}`;
...
// 创建绩效查看弹窗
function createPerformanceModal(userId, userName, tables, permissionLevel) {
    // 创建模态框背景
    ...
    viewButton.addEventListener('click', async () => {
        ...
        try {
            ...
            // 更新结果显示区域
            updatePerformanceResult(resultContainer, performanceData, permissionLevel);
        } catch (error) {
...
// 更新绩效结果显示
function updatePerformanceResult(container, data, permissionLevel) {
    // 清空容器
    container.innerHTML = '';
    ...
    // 根据权限级别显示不同的内容
    // permissionLevel: 1-超级管理员, 2-直接领导, 3-产品线打分人
    ...
    if (permissionLevel <= 2) { // 超级管理员或直接领导可以看到职能评分
        ...
        weightCell.textContent = `${(data.functionalWeight * 100).toFixed(0)}%`;

        const weightedScoreCell = document.createElement('td');
        weightedScoreCell.textContent = (data.functionalScore * data.functionalWeight).toFixed(1);

        functionalRow.appendChild(typeCell);
        functionalRow.appendChild(scoreCell);
        functionalRow.appendChild(weightCell);
        functionalRow.appendChild(weightedScoreCell);
    ...
    productWeightCell.textContent = `${(data.productWeight * 100).toFixed(0)}%`;

    const productWeightedScoreCell = document.createElement('td');
    productWeightedScoreCell.textContent = (data.productScore * data.productWeight).toFixed(1);

    productRow.appendChild(productTypeCell);
    productRow.appendChild(productScoreCell);
    productRow.appendChild(productWeightCell);
    productRow.appendChild(productWeightedScoreCell);

    tbody.appendChild(productRow);

    // 总分行 - 只有超级管理员和直接领导可以看到
    if (permissionLevel <= 2) {
        ...
        totalWeightCell.textContent = '100%';

        const totalWeightedScoreCell = document.createElement('td');
        totalWeightedScoreCell.textContent = data.totalScore.toFixed(1);

        totalRow.appendChild(totalTypeCell);
        totalRow.appendChild(totalScoreCell);
        totalRow.appendChild(totalWeightCell);
        totalRow.appendChild(totalWeightedScoreCell);
